...
import (
	...
	"nsq-proxy/internal/worker"
)

// WorkResponseSuccess 响应成功
...
func (h *Handler) HandleMessage(message *nsq.Message) error {
	...
	_ = json.Unmarshal(message.Body, &ms)
	c := &model.NsqproxyMessage{
		MessageID: ms.ID,
		Topic:     h.consumeConfig.Topic,
		URL:       ms.URL,
		Method:    ms.Method,
		Argument:  ms.Body,
	}
